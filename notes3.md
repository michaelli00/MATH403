...
- **Unity**: nonzero element in ring that is an identity under mulitplication. Doesn't have to exist
...
**Thereom 12.3**: Subring Test - $S$ is a subring if it is closed under subtraction and multiplication $(a, b \in S \implies a-b, ab \in S)$
...
- Let $A$ denote the subset of all polynomials with constant term $0$. Then $A$ is ideal of $\mathbb{R}[x]$ and $A = \langle x \rangle = \{a_1x + \cdots + a_n x^n\}$
...
- Take $I \subseteq Z[x]$. $I$ contains polynomials with an even constant term. Then $I$ is ideal and $I = \langle x, 2 \rangle = \{(a_n x^{n-1} + \cdots + a_1)x + 2k\}$
...
- Proof: We know that cosets form a group under addition so we need to show that multiplication is well defined if and only if $A$ is an ideal of $R$
    ...
    - Suppose $A$ is an ideal of $R$ and let $s + A = s' + A$ and $t + A = t' + A$. We show that $st + A = s't' + A$
        ...
        By definnition we have that $s = s' + a$ and $t = t' + b$ for $a, b \in A$
...
- Ideal $\langle x \rangle$ is prime ideal in $Z[x]$ but is not a maximal ideal in $Z[x]$
    ...
    Thus $g(x)h(x) \in \langle x \rangle \implies g(0)h(0) = 0 \implies g(0) = 0$ or $h(0) = 0$
